pub mod pointer{
    ...
    use std::rc::{Rc,Weak};
    use crate::study::pointer::List1::{Cons1,Nil1};
    use crate::study::pointer::List::{Cons, Nil};
    ...
    pub fn rc(){
        ...
        let list = Rc::new(Cons1(7,Rc::new(Cons1(8,Rc::new(Nil1)))));
    ...
    pub fn week_reference(){
        ...
        //  子级的 parent 是 强引用，branch  的 children 是 弱引用。所以当 branch 被清理时，其子级也会被清理 ，leaf 被清理时，其 parent 不会被清理。
            let leaf = Rc::new(Node{value:5,children:RefCell::new(vec![]),parent:RefCell::new(Weak::new())});
            let branch = Rc::new(Node{value:5,children:RefCell::new(vec![Rc::clone(&leaf)]),parent:RefCell::new(Weak::new())});
